 ...
 */
  ...
  * -------------------------------------------------------------
  */
mba_void bErrorPrint(mba_char* fmt, ...);
mba_void displayBuffer(BufferReader* ptr_Buffer);
mba_long getFileSize(mba_char* fname);
mba_intg isNumber(const mba_char* ns);
mba_void startReader(mba_char*, mba_char*, mba_char, mba_intg, mba_intg);
...
mba_intg mainReader(mba_intg argc, mba_char** argv) {

	/* Create source input buffer */
	mba_char* program = argv[0];
	mba_char* input = argv[2];
	mba_char mode = MODE_FIXED;
	mba_intg size = 0, increment = 0, wrongNumber = 0;
...
************************************************************
*/
mba_void startReader(mba_char* program, mba_char* input, mba_char mode, mba_intg size, mba_intg increment) {

	ReaderPointer bufferp;		/* pointer to Buffer structure */
	FILE* fileHandler;			/* input file handle */
	mba_intg loadSize = 0;		/* the size of the file loaded in the buffer */
	mba_char symbol;			/* symbol read from input file */
...
mba_void bErrorPrint(mba_char* fmt, ...) {
	...
	(mba_void)vfprintf(stderr, fmt, ap);
	va_end(ap);
